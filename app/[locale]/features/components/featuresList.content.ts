import { t } from "intlayer";

const featuresListContent = {
  key: "featuresList",
  content: {
    titlePrefix: t({
      en: "Comprehensive",
      ar: "ميزات",
    }),
    titleHighlight: t({
      en: "Features",
      ar: "شاملة",
    }),
    subtitle: t({
      en: "Everything you need to monitor, analyze, and optimize business performance",
      ar: "كل ما تحتاجه لمراقبة أداء الأعمال وتحليله وتحسينه",
    }),

    feature1Title: t({ en: "Real-Time Analytics", ar: "تحليلات فورية" }),
    feature1Description: t({
      en: "Monitor KPIs and performance metrics across your entire organization with live dashboards.",
      ar: "راقب مؤشرات الأداء الرئيسية ومقاييس الأداء عبر مؤسستك بالكامل من خلال لوحات معلومات مباشرة.",
    }),

    feature2Title: t({ en: "Predictive Modeling", ar: "نمذجة تنبؤية" }),
    feature2Description: t({
      en: "AI-powered forecasting helps you anticipate trends and make proactive decisions.",
      ar: "يساعدك التنبؤ المدعوم بالذكاء الاصطناعي على توقع الاتجاهات واتخاذ قرارات استباقية.",
    }),

    feature3Title: t({ en: "Unified Data Hub", ar: "مركز بيانات موحّد" }),
    feature3Description: t({
      en: "Consolidate data from all systems into one centralized platform for complete visibility.",
      ar: "وحّد بيانات جميع الأنظمة في منصة مركزية واحدة لشفافية كاملة.",
    }),

    feature4Title: t({ en: "Workflow Automation", ar: "أتمتة سير العمل" }),
    feature4Description: t({
      en: "Automate reporting, approvals, and routine processes to save time and reduce errors.",
      ar: "أتمت التقارير والموافقات والعمليات الروتينية لتوفير الوقت وتقليل الأخطاء.",
    }),

    feature5Title: t({
      en: "Enterprise Security",
      ar: "أمن على مستوى المؤسسة",
    }),
    feature5Description: t({
      en: "Bank-grade security with role-based access controls and compliance features.",
      ar: "أمان بمستوى البنوك مع صلاحيات وصول حسب الأدوار وميزات امتثال.",
    }),

    feature6Title: t({ en: "Performance Tracking", ar: "تتبّع الأداء" }),
    feature6Description: t({
      en: "Track team, department, and company-wide performance against strategic goals.",
      ar: "تتبّع أداء الفرق والأقسام والشركة بالكامل مقابل الأهداف الاستراتيجية.",
    }),
  },
};

export default featuresListContent;
