...
export default function EPM() {
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        if (!formData.name.trim() || !formData.company.trim() || !formData.email.trim() || !formData.phone.trim() || !formData.epm.trim()) {
            showTempMsg(setFormError, content.errorFillRequired);
            return;
        }

        // Validate full name (only letters and spaces)
        if (!nameRegex.test(formData.name.trim())) {
            showTempMsg(setFormError, content.errorNameOnlyLetters);
            return;
        }

        // Validate name length (at least 3 characters)
        if (formData.name.trim().length < 3) {
            showTempMsg(setFormError, content.errorNameMinLength);
            return;
        }

        // Validate company name (at least 3 characters)
        if (formData.company.trim().length < 3) {
            showTempMsg(setFormError, content.errorCompanyMinLength);
            return;
        }

        // Validate email format
        if (!emailRegex.test(formData.email.trim())) {
            showTempMsg(setFormError, content.errorInvalidEmail);
            return;
        }
        ...
        if (phoneDigitsOnly.length < 10 || phoneDigitsOnly.length > 15) {
            showTempMsg(setFormError, content.errorInvalidPhone);
            return;
        }
        setSubmitting(true);
        try {
            ...
            showTempMsg(setSuccessMsg, content.successSubmitted);
            setFormData({
                name: '',
                company: '',
                email: '',
            ...
            showTempMsg(setFormError, content.errorSubmitFailed);
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div id='epm-suites' className="bg-[#0F1E4D] flex items-center justify-center py-12 md:py-20 lg:py-24 px-6 relative overflow-hidden">
            ...
            <div className='absolute left-0 top-0'>
                <Image src='/left.svg' alt='left image' width={150} height={150} />
            </div>

            <div className='absolute right-0 bottom-0'>
                <Image src='/right.svg' alt='right image' width={150} height={150} />
            </div>
                ...
                <div className="text-white space-y-6">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-blue-100">{content.bullet1}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-blue-100">{content.bullet2}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-blue-100">{content.bullet3}</span>
                        </div>
                    </div>
                </div>

                {/* Right side - Form */}
                <div className="bg-white rounded-[20px] md:rounded-[40px] shadow-2xl p-6 md:p-8">
                    ...
                    <div className="space-y-4">
                        ...
                        <label className="block text-gray-700 font-medium mb-1">
                            {content.workEmailLabel} <span className="text-red-500">*</span>
                        </label>
                        <input
                            type="email"
                            name="email"
                        ...
                        <label className="block text-gray-700 font-medium mb-1">
                            {content.companyNameLabel} <span className="text-red-500">*</span>
                        </label>
                        <input
                            type="text"
                            name="company"
                        ...
                        <label className="block text-gray-700 font-medium mb-1">
                            {content.phoneNumberLabel} <span className="text-red-500">*</span>
                        </label>
                        <input
                            type="tel"
                            name="phone"
                        ...
                        <label htmlFor="epm-service" className="block text-gray-700 font-medium mb-1">
                            {content.serviceLabel} <span className="text-red-500">*</span>
                        </label>
                        <select
                            id="epm-service"
                            name="epm"
                            value={formData.epm}
                            onChange={handleChange}
                            className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition resize-none"
                        >
                            <option value="">{content.servicePlaceholder}</option>
                            <option value="fcc">{content.optionFcc}</option>
                            <option value="planning&budgeting">{content.optionPlanning}</option>
                            <option value="account-reconciliation">{content.optionAccountReconciliation}</option>
