...
const SocialMediaHero = () => {
    ...
    const content = useIntlayer("socialMediaPage");
            ...
            nodes.forEach((n, i) => {
                ...
                nodes.slice(i + 1).forEach((n2) => {
                    ...
                    if (distance < 140) {
                        ...
                        ctx.strokeStyle = `rgba(79, 70, 229, ${(1 - distance / 140) * 0.2})`;
                        ctx.lineWidth = 1;
                        ctx.stroke();
                    }
                });
    ...
    return (
        <>
            ...
            <section className="relative overflow-hidden bg-gradient-to-br from-[#050A1E] via-[#0D0B1F] to-[#0A0A14] min-h-screen flex items-center">
                ...
                >
                    <div className="inline-flex items-center gap-3 px-6 py-3 mb-10 rounded-full bg-white/5 border border-indigo-500/20 backdrop-blur-xl relative overflow-hidden">
                        ...
                        <span className="relative text-indigo-400 text-sm font-semibold tracking-wide">
                    ...
                    <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-bold text-white mb-8 leading-tight" style={{ fontFamily: "var(--heading-font)" }}>
                        ...
                        <span className="relative inline-block mt-4">
                            <span 
                                className="bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 bg-clip-text text-transparent"
                                style={{ backgroundSize: '200% auto', animation: 'gradient-flow 5s ease infinite' }}
                            >
                                {content.heroTitleLine2.value}
                            </span>
                            <div className="absolute -bottom-3 left-0 right-0 h-px bg-gradient-to-r from-transparent via-indigo-500 to-transparent overflow-hidden">
                    ...
                    <div className="flex flex-col sm:flex-row gap-6 justify-center mb-20">
                        <button
                            ...
                            className="group relative px-12 py-5 bg-indigo-600 text-white rounded-2xl text-lg font-semibold overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-indigo-600/40 hover:-translate-y-1"
                        >
                        ...
                        <button
                            onClick={(e) => {
                                const link = "#social-services";
                                if (!handleHashLink(e, link, router)) {
                                    const servicesSection = document.getElementById('social-services');
                                    servicesSection?.scrollIntoView({ behavior: 'smooth' });
                                }
                            }}
                            className="group relative px-12 py-5 bg-transparent text-white border-2 border-white/20 rounded-2xl text-lg font-semibold backdrop-blur-sm overflow-hidden transition-all duration-500 hover:border-indigo-500/50 hover:-translate-y-1"
                        >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 sm:gap-12 max-w-4xl mx-auto">
                        {[
                            ...
                            <div key={i} className="group relative">
                                <div className="text-4xl sm:text-5xl md:text-6xl font-bold bg-gradient-to-br from-indigo-400 to-purple-600 bg-clip-text text-transparent mb-3 transition-transform duration-300 group-hover:scale-110">
                                    {stat.value}
                                </div>
                                <div className="text-xs sm:text-sm text-white/50 uppercase tracking-widest font-light">
                                    {stat.label}
                                </div>
                                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-px bg-indigo-500 group-hover:w-full transition-all duration-500" />
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="absolute bottom-12 left-1/2 -translate-x-1/2">
                    <div className="flex flex-col items-center gap-3 animate-bounce">
                        <span className="text-indigo-500/50 text-xs uppercase tracking-widest font-light">
...
const SocialServices = () => {
    ...
    return (
        <section id="social-services" className="py-24 bg-gradient-to-b from-[#0A0A14] to-[#050A1E]">
            <div className="max-w-7xl mx-auto px-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {services.map((service, index) => (
                        <div
                            key={index}
                            className="group relative p-8 rounded-3xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-indigo-500/30 transition-all duration-500 hover:-translate-y-2"
                        >
